...
/**
 * TVBox PHP 爬虫脚本
 * 支持JSON/TXT/M3U/DB/PHP/多媒体文件格式 + 在线视频采集
    ...
    public function getCategories() {
        ...
        foreach ($patterns as $pattern) {
            ...
            foreach ($matches as $match) {
                ...
                // 移除苹果和一个区的资源
                if (strpos($name, '苹果') !== false || strpos($name, '一个区') !== false) {
                    continue;
                }
...
function getAllFiles() {
    ...
    if ($allFiles === null) {
        ...
        $phpFiles = array_merge(
            scanDirectoryRecursive('/storage/emulated/0/江湖/php/', ['php']),
            scanDirectoryRecursive('/storage/emulated/0/江湖/json/影视/', ['php']),
            scanDirectoryRecursive('/storage/emulated/0/江湖/wj/', ['php'])
        );
        ...
        $allFiles = array_merge($jsonFiles, $txtFiles, $m3uFiles, $dbFiles, $phpFiles, $mediaFiles);
...
function getCategories() {
    ...
    if ($categories === null) {
        ...
        $fileTypes = [
            'txt' => ['name' => '📄 TXT文件', 'icon' => '📄'],
            'json' => ['name' => '📊 JSON文件', 'icon' => '📊'],
            'm3u' => ['name' => '📺 M3U文件', 'icon' => '📺'],
            'db' => ['name' => '🗃️ 数据库文件', 'icon' => '🗃️'],
            'php' => ['name' => '🐘 PHP文件', 'icon' => '🐘'],
            'media' => ['name' => '🎬 本地视频', 'icon' => '🎬']
        ];
        ...
        foreach ($allFiles as $file) {
            $fileType = $file['type'];
            if (in_array($fileType, ['txt', 'json', 'm3u', 'db', 'php'])) {
                $typeFiles[$fileType][] = $file;
            } elseif (in_array($fileType, ['mp4', 'mkv', 'avi', 'mov', 'wmv', 'flv', 'webm', 'm4v', '3gp'])) {
                $typeFiles['media'][] = $file;
            }
        }
        
        // 添加类型分类
        foreach ($fileTypes as $type => $typeInfo) {
            $fileCount = count($typeFiles[$type]);
            if ($fileCount > 0) {
                $totalVideos = 0;
                foreach ($typeFiles[$type] as $file) {
                    $totalVideos += estimateFileVideoCount($file);
                }
                
                $categories[] = [
                    'type_id' => 'type_' . $type,
                    'type_name' => $typeInfo['icon'] . ' ' . $typeInfo['name'] . ' (' . $fileCount . '个文件, ' . $totalVideos . '个视频)',
                    'type_file' => $type . '_category',
                    'source_path' => $type,
...
function parsePhpFile($filePath) {
    if (!file_exists($filePath)) {
        return ['error' => 'PHP文件不存在: ' . $filePath];
    }
    
    // 文件大小检查（1024M）
    $fileSize = filesize($filePath);
    if ($fileSize > 1024 * 1024 * 1024) {
        return ['error' => 'PHP文件过大: ' . round($fileSize / (1024 * 1024), 2) . 'MB'];
    }
    
    $phpContent = @file_get_contents($filePath);
    if ($phpContent === false) {
        return ['error' => '无法读取PHP文件: ' . $filePath];
    }
    
    // 检查是否为合法的PHP源文件（包含常见的TVBox源格式）
    $videos = [];
    
    // 尝试解析PHP文件中的JSON数据
    if (preg_match('/\$url\s*=\s*[\'"]([^\'"]+)[\'"]/', $phpContent, $matches)) {
        $url = $matches[1];
        // 如果是远程PHP源，尝试获取内容
    ...
    // 尝试直接解析PHP文件中的数组或JSON字符串
    if (preg_match('/json_decode\s*\(\s*[\'"]([^\'"]+)[\'"]\s*\)/', $phpContent, $matches)) {
    ...
    // 尝试查找数组定义
    if (preg_match('/\$[\w]+\s*=\s*\[.*?\]\s*;/s', $phpContent, $matches)) {
        // 这里简化处理，实际应该更复杂的解析
        $defaultImages = [
        ...
        $videos[] = [
            'vod_id' => 'php_' . md5($filePath),
            'vod_name' => 'PHP源: ' . $fileName,
            'vod_pic' => $defaultImages[0],
            'vod_remarks' => 'PHP源',
            'vod_year' => date('Y'),
            'vod_area' => '网络',
            'vod_content' => 'PHP视频源文件: ' . $fileName,
            'vod_play_from' => 'PHP播放',
    ...
    return ['error' => '无法解析PHP文件格式: ' . $filePath];
}
...
function getCategory($tid, $pg) {
    ...
    // 文件类型分类处理（TXT、JSON、M3U、DB、PHP、多媒体）
    if (strpos($tid, 'type_') === 0) {
        $fileType = str_replace('type_', '', $tid);
        $allFiles = getAllFiles();
        ...
        foreach ($typeFiles as $file) {
            ...
            $typeVideos = array_merge($typeVideos, $videos);
            
            // 内存保护
            if (count($typeVideos) > 1000) {
                break;
            }
        }
        
        if (empty($typeVideos)) {
            return ['error' => '在该类型中未找到视频: ' . $fileType];
        }
        
        // 分页处理
        $pageSize = 10;
        $total = count($typeVideos);
        ...
        $pagedVideos = array_slice($typeVideos, $start, $pageSize);
    ...
    if (file_exists($targetCategory['source_path'])) {
        switch ($targetCategory['source_type']) {
            ...
            case 'db':
                ...
                break;
            case 'php':
                $categoryVideos = parsePhpFile($targetCategory['source_path']);
                break;
            case 'mp4':
            case 'mkv':
            case 'avi':
...
function findVideoById($id) {
    ...
    } elseif (strpos($id, 'php_') === 0) {
        $parts = explode('_', $id);
        if (count($parts) >= 2) {
            $fileHash = $parts[1];
            
            foreach ($allFiles as $file) {
                if ($file['type'] === 'php' && md5($file['path']) === $fileHash) {
                    $videos = parsePhpFile($file['path']);
                    return $videos[0] ?? null;
                }
            }
