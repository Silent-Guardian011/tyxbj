<?php
/**
 * TVBox PHP 爬虫脚本 - 四合一综合版（优化版）
 * 支持本地文件(JSON/TXT/M3U/DB) + 苹果视频 + 金牌影视 + LSP传媒
 * 按类型分类归拢显示，12宫格布局
 */
...
switch ($ac) {
    case 'detail':
        if ($source === 'apple') {
            ...
            echo json_encode(getLspDetail($ids), JSON_UNESCAPED_UNICODE);
        } else {
            if (!empty($ids)) {
    ...
    case 'search':
        if ($source === 'apple') {
            ...
            echo json_encode(searchLsp($wd, $pg), JSON_UNESCAPED_UNICODE);
        } else {
    ...
    case 'play':
        if ($source === 'apple') {
            ...
            echo json_encode(getLspPlay($flag, $id), JSON_UNESCAPED_UNICODE);
        } else {
    ...
    case 'home':
        if ($source === 'apple') {
            ...
            echo json_encode(getLspHome(), JSON_UNESCAPED_UNICODE);
        } else {
            echo json_encode(getHome(), JSON_UNESCAPED_UNICODE);
        }
        break;
...
function getCategory($tid, $pg) {
    ...
    if ($tid === 'lsp') {
        return getLspCategory('lsp_1', $pg, []);
    }
    
    // LSP传媒具体分类处理
    if (strpos($tid, 'lsp_') === 0) {
        return getLspCategory($tid, $pg, []);
    }
            ...
            if (preg_match_all($pattern, $htmlContent, $matches)) {
                foreach ($matches[1] ?? $matches[0] as $match) {
                    ...
                    if (strpos($extractedUrl, '//') === 0) {
                        ...
                        $extractedUrl = $this->host . '/' . ltrim($extractedUrl, '/');
                    }
...
class LspCmSpider {
    ...
    public function getCategoryContent($tid, $pg, $extend) {
        ...
        try {
            // 提取分类ID
            $typeId = str_replace('lsp_', '', $tid);
            $url = $this->siteUrl . "/index.php/vod/type/id/{$typeId}/page/{$pg}.html";
            $rsp = $this->fetch($url);
            ...
            if ($rsp) {
                ...
                foreach ($items as $item) {
                    ...
                    if ($hrefMatch && $titleMatch) {
                        ...
                        // 生成视频ID
                        $videoId = 'lsp_' . $typeId . '_' . abs(crc32($href));
                        
                        $videos[] = array(
                            'vod_id' => $videoId,
                            'vod_name' => $title,
                            'vod_pic' => $img ?: getRandomImage($videoId),
                            'vod_remarks' => $desc,
                            'vod_year' => ''
                        );
                    }
    ...
    public function getDetailContent($ids) {
        ...
        try {
            ...
            if (count($parts) >= 3) {
                ...
                // 由于我们无法从hash还原原始URL，这里需要重新获取列表来匹配
                $url = $this->siteUrl . "/index.php/vod/type/id/{$typeId}/page/1.html";
                $rsp = $this->fetch($url);
                ...
                if ($rsp) {
                    ...
                    foreach ($items as $item) {
                        ...
                        if ($hrefMatch && $titleMatch) {
                            $currentHash = abs(crc32($hrefMatch[1]));
                            if ($currentHash == $videoHash) {
                                $detailUrl = $this->siteUrl . (strpos($hrefMatch[1], '/') === 0 ? $hrefMatch[1] : '/' . $hrefMatch[1]);
                                $detailRsp = $this->fetch($detailUrl);
                                
                                if ($detailRsp) {
                                    // 提取视频标题
                                    preg_match('/<h1[^>]*class="[^"]*section-header__title[^"]*"[^>]*>(.*?)<\/h1>/', $detailRsp, $titleMatch);
                                    // 提取封面图片
                                    preg_match('/<div[^>]*class="[^"]*xgplayer-poster[^"]*"[^>]*style="[^"]*background-image:\s*url\(\'?([^\'\)]+)/', $detailRsp, $picMatch);
                                    // 提取描述
                                    preg_match('/<div[^>]*class="[^"]*video-footer__description[^"]*"[^>]*>(.*?)<\/div>/s', $detailRsp, $descMatch);
                                    
                                    $title = $titleMatch ? trim($titleMatch[1]) : $titleMatch[1];
                                    $pic = $picMatch ? $picMatch[1] : ($imgMatch ? $imgMatch[1] : getRandomImage($vid));
                                    $desc = $descMatch ? trim(strip_tags($descMatch[1])) : '';
                                    
                                    // 获取播放地址
                                    $playUrl = $this->extractPlayUrl($detailRsp);
                                    ...
                                    $vod = array(
                                        ...
                                        'vod_remarks' => '',
                                        'vod_content' => $desc,
                                        'vod_play_from' => $playUrl ? '默认线路' : 'LSP传媒',
                                        'vod_play_url' => $playUrl ? '正片$' . $playUrl : "正片\${$detailUrl}"
                                    );
    ...
    public function getSearchContent($key, $pg = 1) {
        ...
        try {
            ...
            $encodedKey = urlencode($key);
            $url = $this->siteUrl . "/index.php/vod/search/wd/{$encodedKey}/page/{$pg}.html";
            $rsp = $this->fetch($url);
            ...
            if ($rsp) {
                ...
                foreach ($items as $item) {
                    ...
                    if ($hrefMatch && $titleMatch) {
                        $videoId = 'lsp_search_' . abs(crc32($hrefMatch[1]));
                        $videos[] = array(
                            'vod_id' => $videoId,
                            'vod_name' => $titleMatch[1],
                            'vod_pic' => $imgMatch ? $imgMatch[1] : getRandomImage($videoId),
 ...
 * LSP传媒分类
 */
function getLspCategory($tid, $pg, $extend) {
    $spider = new LspCmSpider();
    return $spider->getCategoryContent($tid, $pg, $extend);
}

/**
 * LSP传媒详情
 */
function getLspDetail($ids) {
    $spider = new LspCmSpider();
    return $spider->getDetailContent(explode(',', $ids));
}
 ...
 * LSP传媒搜索
 */
function searchLsp($keyword, $page) {
    $spider = new LspCmSpider();
    return $spider->getSearchContent($keyword, $page);
}
