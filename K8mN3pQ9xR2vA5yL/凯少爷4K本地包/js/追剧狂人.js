...
一级: $js.toString(() => {
    let klist = pdfa(request(input), '.public-list-box');
    //数组
    let k = klist.map(it => ({
...
二级: $js.toString(() => {
    ...
    VOD = {
        vod_id: input,
        vod_name: pdfh(html, 'h3&&Text').replace(/\d+\.\d+$/,''), // 剔除标题后评分（当前标题无评分，保留逻辑）
        type_name: pdfh(html, 'li:contains(类型)&&Text'), // 匹配类型对应的链接文本
        vod_pic: pd(html, 'img&&data-src', input), // 匹配懒加载图片的真实地址（无则返回input兜底）
        vod_remarks: pdfh(html, 'li:contains(状态)&&Text'), // 匹配状态（第6集）
        vod_year: pdfh(html, 'li:contains(年份)&&Text').replace('年份：','').trim(), // 提取年份文本
        vod_area: pdfh(html, 'li:contains(地区)&&Text').replace('地区：','').trim(), // 提取地区文本
        vod_director: pdfh(html, 'li:contains(导演)&&Text') || '未知', // 匹配导演链接，无则显示未知
        vod_actor: pdfh(html, 'li:contains(演员)&&Text').replace('演员：','').replace(/<a[^>]+>/g,'').replace(/<\/a>/g,'').replace(/&nbsp;/g,' ').trim(), // 提取主演并去标签
        vod_content: pdfh(html, '.switch-box.top20&&Text').trim() // 提取完整剧情文本
    };


         let r_ktabs = pdfa(html,'.anthology-tab&&.swiper-wrapper&&a');
         //线路列表
...
let klists = [];
let r_plists = pdfa(html, '.anthology-list-box');
r_plists.forEach((rp) => {
