...
class Spider(Spider):
    ...
    def init(self, extend=""):
        ...
        self.scanned_paths = set() # 引入物理路径去重集合，防止软链接及重复挂载引起的死循环
    ...
    def _scan_with_depth(self, current_dir, current_depth):
        ...
        # 通过真实的物理绝对路径防止跨目录挂载造成的重复扫描
        try:
            real_current_path = os.path.realpath(current_dir)
            if real_current_path in self.scanned_paths:
                return
            self.scanned_paths.add(real_current_path)
        except Exception:
            pass
        ...
        try:
            for file in os.listdir(current_dir):
                ...
                if os.path.isdir(full_path):
                    ...
                    # 🚀 【第一重排重】如果该绝对路径已经记录在库中，直接跳过，防止产生任何形式的双胞胎
                    if any(d.get("path") == abs_path for d in self.databases.values()):
                        continue

                    db_key = f"auto_{file}"
                    
                    # 🚀 【第二重排重】如果构建的 db_key 已经存在（说明同名文件由于跨目录等原因重复扫描到了），直接跳过
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        if self._is_exclusive_db(conn):
            ...
            # 普通数据库，走完全无损还原的多级树状类目缓存显示（支持图片封面加载）
    ...
    def _legacy_category(self, conn, db_key, parts, pg):
        """无损还原：纯净的多级树状类目层，支持逐级无限嵌套、完美读取 avatar_map 封面图片"""
        ...
        if cache_key not in self._db_cache:
            ...
            filter_field = "type_name" if "type_name" in all_cols else auto_info["field_mapping"].get("category_field")
            if not filter_field:
                filter_field = "type_name"
    ...
    def _get_auto_mapping(self, conn):
        try:
            ...
            # 2. 按照最高优先级匹配最核心的视频数据表
            target_table = next((t for t in ["videos", "vod_unified_data", "cj", "vod", "video", "movies", "data", "video_detail"] if t in tables), None)
            
            # 3. 如果一个都不认识，就暴力抓取第一个非系统专属表（盲扫保底）
            if not target_table and tables:
                target_table = tables[0]
            
            if not target_table: return None
            
            # 4. 融合双重字典候选词，提升全局动态匹配字段的精度
            cursor.execute(f"PRAGMA table_info(`{target_table}`)")
            cols = [str(r[1]).lower() for r in cursor.fetchall()]
            
            mapping = {}
            field_candidates = {
                "vod_id": ["id", "vod_id", "uuid", "guid", "vid", "movie_id"],
                "vod_name": ["name", "vod_name", "title", "subject", "display_name", "movie_name"],
                "vod_pic": ["image", "vod_pic", "pic", "pic_url", "thumbnail", "img", "cover"],
                "vod_play_url": ["play_url", "vod_play_url", "url", "link", "m3u8_url", "urls", "links"],
                "vod_remarks": ["vod_remarks", "remarks", "note", "desc"],
                "category_field": ["type_name", "category_id", "class_name", "class_id", "cate_name", "actress_id", "tag", "type"],
            ...
            for target_field, candidates in field_candidates.items():
                matches = [cand for cand in candidates if cand.lower() in cols]
                mapping[target_field] = matches[0] if matches else None
                
            # 针对老版本简易字典名称的映射包进行双向兜底，确保向下完全兼容
            if not mapping.get("id"): mapping["id"] = mapping.get("vod_id") or "id"
            if not mapping.get("name"): mapping["name"] = mapping.get("vod_name") or "name"
            if not mapping.get("pic"): mapping["pic"] = mapping.get("vod_pic") or "pic"
            if not mapping.get("urls"): mapping["urls"] = mapping.get("vod_play_url") or "urls"
    ...
    def _legacy_detail(self, conn, db_key, real_id):
        ...
        # 📝 保留原有简介内容，在末尾无损追加清单统计和物理路径
        base_content = get_val("vod_content") or get_val("vod_remarks") or "暂无简介"
        db_path = self.databases.get(db_key, {}).get("path", "未知")
        extended_content = f"{base_content}\n[清单统计] 本库包含影片数: {total_count} 条\n[数据源路径] {db_path}"
        ...
        vod = {
            ...
            "vod_content": extended_content, # 注入升级版简介
    ...
    def _exclusive_detail(self, conn, db_key, vid_or_name):
        ...
        # 📝 保留原有简介内容，在末尾无损追加清单统计和物理路径
        base_content = row["vod_content"] or ""
        db_path = self.databases.get(db_key, {}).get("path", "未知")
        extended_content = f"{base_content}\n\n[清单统计] 本库包含影片数: {total_count} 条\n[数据源路径] {db_path}"
