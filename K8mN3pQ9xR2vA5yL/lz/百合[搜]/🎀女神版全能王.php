    ...
    foreach ($目录项 as $项目) {
        ...
        if (is_dir($路径)) {
            ...
            if (in_array($扩展名, $文件类型)) {
                $相对路径 = str_replace('/storage/emulated/0/lz/XXX/', '', $路径);
                
                // 检查是否为磁力文件夹
                $是磁力文件夹 = (strpos($路径, '/lz/XXX/bt/') !== false);
...
function 获取所有文件() {
    ...
    if ($所有文件 === null) {
        ...
        // 统一扫描根目录及所有子目录中的所有支持文件类型
        $所有文件 = 递归扫描目录(
            '/storage/emulated/0/lz/XXX/',
            ['json', 'txt', 'm3u', 'db', 'sqlite', 'sqlite3', 'db3']
        );
