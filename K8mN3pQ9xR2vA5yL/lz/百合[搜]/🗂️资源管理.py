...
# ==================== 路径配置 ====================
ROOT_PATHS = [
    '/storage/emulated/0/lz/',
    '/storage/emulated/0/lz/AAA/',
    '/storage/emulated/0/lz/XXX/'
  
]

PATH_TO_CHINESE = {
    '/storage/emulated/0/lz/': 'lz',
    '/storage/emulated/0/lz/AAA/': '绿色合集',
    '/storage/emulated/0/lz/XXX/': '女神合集'
}
...
print("ℹ️ 本地资源管理加载成功 - 完整修复版 + 修复JSON显示格式和TXT直播源识别")
...
class Spider(Spider):
    ...
    def _create_file_item(self, f):
        ...
        if f['is_dir']:
            return {
                ...
                'vod_pic': self._get_random_image(hash(f['path'])),
                'vod_remarks': '文件夹',
                'vod_tag': 'folder',
                'style': {'type': 'list'}
            }
        ...
        if self.is_audio_file(f['ext']):
            return {
                ...
                'vod_tag': 'audio',
                'style': {'type': 'list'},
                'vod_player': '听'  # 设置为 "听"，表示音频播放模式
            }
        
        # 3. 图片文件 - 添加 vod_player 字段，值为 "画"
        ...
        # 10. TXT文件 - 识别为直播源或文本文件
        if f['ext'] == 'txt':
            # 读取文件内容判断是否包含播放链接
            has_playable_urls = False
            url_count = 0
                        ...
                        if ',' in line:
                            ...
                            if len(parts) == 2:
                                ...
                                if url.startswith(('http://', 'https://', 'rtmp://', 'rtsp://')):
                                    # 检查是否是视频流地址
                                    if any(ext in url.lower() for ext in ['.m3u8', '.m3u', '.flv', '.mp4', '.ts', '.mkv', '.avi']):
                                        url_count += 1
                                    elif re.match(r'https?://[\d.]+:\d+', url):
                                        url_count += 1
                        # 检测直接URL
                        elif line.startswith(('http://', 'https://', 'rtmp://', 'rtsp://')):
                            if any(ext in line.lower() for ext in ['.m3u8', '.m3u', '.flv', '.mp4', '.ts', '.mkv', '.avi']):
                                url_count += 1
                        # 检测P2P链接
                        elif 'ed2k://' in line.lower() or 'magnet:?' in line.lower():
                            url_count += 1
                        if url_count >= 1:
                            has_playable_urls = True
                            break
            except Exception as e:
                self.log(f"读取TXT文件失败: {e}")
            
            if has_playable_urls:
                # 包含播放链接的TXT文件 - 当作直播源处理
                return {
                    'vod_id': self.LIST_PREFIX + self.b64u_encode(f['path']),
                    'vod_name': f"📺 {f['name']}",
                    'vod_pic': self._get_random_image(hash(f['path'])),
                    'vod_play_url': f"播放${self.LIST_PREFIX + self.b64u_encode(f['path'])}",
                    'vod_remarks': f'播放列表 ({url_count}个链接)',
                    'vod_tag': 'live_txt',
                    'style': {'type': 'list'}
                }
            else:
                    ...
                    if ',#genre#' in content.lower():
                        ...
                        for line in lines[:30]:
                            ...
                            if ',' in line:
                                ...
                                if len(parts) == 2:
                                    ...
                                    if url.startswith(('http://', 'https://')):
                                        if any(ext in url.lower() for ext in ['.m3u8', '.mp4', '.flv', '.ts', '.mkv', '.avi']):
                                            url_count += 1
                                        elif re.match(r'https?://[\d.]+:\d+', url):
                                            url_count += 1
                            ...
                            # 检测直接URL
                                ...
                                if any(ext in line_lower for ext in ['.m3u8', '.mp4', '.flv', '.ts', '.mkv', '.avi']):
                                    url_count += 1
    ...
    def _create_fallback_vod(self, file_path, file_type, vod_id, name=None):
        return {
            'vod_id': vod_id,
            'vod_name': os.path.basename(file_path),
            'vod_pic': self._get_random_image(hash(file_path)),
    ...
    def _handle_audio_all_detail(self, dir_path, vod_id):
        ...
        poster = self._get_random_image(hash(dir_path))
        if audios:
    ...
    def _handle_video_all_detail(self, dir_path, vod_id):
        ...
        return {'list': [{
            ...
            'vod_pic': self._get_random_image(hash(dir_path)),
            'vod_play_from': '本地视频',
            'vod_play_url': '#'.join(play_urls),
    ...
    def _handle_file_detail(self, file_path):
        ...
        if self.is_image_file(ext):
            ...
            if len(all_audios) > 1:
                ...
                if not poster:
                    poster = self._get_random_image(hash(file_path))
    ...
    def _live_source_detail(self, source_id):
        ...
        for line_idx in range(max_lines):
            ...
            from_list.append(line_name)  # 线路名称 - 显示在上面
            ...
            for channel_name, links in channels.items():
                if line_idx < len(links):
                    ...
                    # 这条线路没有这个频道，用空占位（或者可以跳过）
                    # 这里选择不添加，让线路只显示实际有的频道
                    pass
        ...
        vod = {
            ...
            'vod_name': source['name'],
            'vod_pic': icon,
            'vod_play_from': '$$$'.join(from_list),  # 线路名称（现在显示在上面）
            'vod_play_url': '$$$'.join(url_list),     # 电视台列表（现在显示在下面）
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if url.startswith('file://'):
            ...
            if os.path.exists(file_path) and self.is_audio_file(self.get_file_ext(file_path)):
                ...
                result["vod_player"] = "听"  # 设置为 "听"，表示音频播放模式
        
        # 图片文件处理
        if url.startswith('file://') and self.is_image_file(self.get_file_ext(url[7:])):
            result["vod_player"] = "画"  # 设置为 "画"，表示图片浏览模式
    ...
    # 直接播放URL - 新增函数
    def 直接播放URL(self, 播放URL):
        """直接播放URL"""
        # 去除空格和反引号
        播放URL = 播放URL.strip(' `')
        
        # 解码URL
        播放URL = urllib.parse.unquote(播放URL)
        ...
        # 检查是否是电影天堂的分享链接
        if 'vip.dytt-' in 播放URL and '/share/' in 播放URL:
            # 对于电影天堂的分享链接，需要设置为需要解析
            是否需要解析 = 1
            播放类型 = 'video'
        elif 播放URL.startswith('magnet:'):
            播放类型 = 'magnet'
            是否需要解析 = 0
        elif 播放URL.startswith('ed2k://'):
            播放类型 = 'ed2k'
            是否需要解析 = 0
        elif '.m3u8' in 播放URL:
            播放类型 = 'hls'
            是否需要解析 = 0
        elif any(ext in 播放URL for ext in ['.mp4', '.avi', '.mkv']):
            播放类型 = 'video'
            是否需要解析 = 0
        
        # 解析URL获取Referer
        from urllib.parse import urlparse
        parsed_url = urlparse(播放URL)
...
def main():
    ...
    操作类型 = params.get('ac', 'detail')
    分类标识 = params.get('t', '')
    页码 = params.get('pg', '1')
    视频标识 = params.get('ids', '')
    搜索关键词 = params.get('wd', '')
    播放标志 = params.get('flag', '')
    播放标识 = params.get('id', '')
    播放URL = params.get('play', '')  # 新增 play 参数
    
    # 根据不同 action 返回数据
    spider = Spider()
    if 操作类型 == 'detail':
        if 视频标识:
            结果 = spider.detailContent([视频标识])
        elif 分类标识:
            结果 = spider.categoryContent(分类标识, 页码, None, None)
        else:
            结果 = spider.homeContent(None)
    elif 操作类型 == 'search':
        结果 = spider.searchContent(搜索关键词, False, 页码)
    else:
        结果 = {'错误': f'未知操作: {操作类型}'}
    
    # 新增：直接播放URL的处理
    if 播放URL:
        结果 = spider.直接播放URL(播放URL)
