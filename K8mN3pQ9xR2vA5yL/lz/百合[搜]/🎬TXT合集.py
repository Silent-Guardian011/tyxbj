...
class Spider(Spider):
    ...
    def init(self, extend):
        ...
        roots = [extend.strip()] if extend else ["/storage/emulated/0/lz/AAA", "/storage/emulated/0/lz/XXX", "/storage"]
        all_cats, all_index = [], {}
        unique_roots = set()
    ...
    def _deep_scan(self, path, is_int, all_cats, all_index):
        try:
            scan_dirs = []
            for sub in ["AAA", "XXX"]:
                wj_path = os.path.join(path, sub, "txt")
                if os.path.isdir(wj_path):
                    scan_dirs.append(wj_path)
            if not scan_dirs:
                scan_dirs = [path]
                        ...
                        for sub in os.scandir(entry.path):
                            if sub.is_dir():
                                ...
                                if sub_f:
                                    ...
                                    type_name = f"{sub.name}" if entry.name == "txt" else f"{entry.name}/{sub.name}"
                                    all_cats.append({"type_id": sid, "type_name": type_name})
        ...
        for f_path in file_list:
            ...
            for i in range(parts):
                ...
                v_list.append({
                    ...
                    "vod_pic": self._get_random_image(len(v_list)), "vod_remarks": info['rem']
                })
