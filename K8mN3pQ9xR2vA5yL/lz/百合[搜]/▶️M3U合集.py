...
class Spider(Spider):
    ...
    def init(self, extend):
        ...
        raw_roots = [extend.strip()] if extend else ["/storage/emulated/0/lz", "/storage"]
        ...
        for r in raw_roots:
            ...
            for target in scan_targets:
                for sub in ["AAA", "XXX"]:
                    m3u_path = os.path.join(target, sub, "m3u")
                    if os.path.isdir(m3u_path) and m3u_path not in self.scanned_paths:
                        self.scanned_paths.add(m3u_path)
                        suffix = "" if is_int else "☆"
                        
                        # 处理 m3u 目录下直接的文件
                        root_files = [e.path for e in os.scandir(m3u_path) if not e.is_dir() and e.name.lower().endswith(('.m3u', '.txt'))]
                        if root_files:
                            tid = base64.b64encode(f"C|{m3u_path}".encode()).decode()
                            all_raw_cats.append({"type_id": tid, "type_name": f"{sub}{suffix}"})
                            final_index[tid] = sorted(list(set(root_files)))
