...
class Spider extends BaseSpider
{
    ...
    private $ROOT = [
        ...
        '/storage/emulated/0/lz/音乐[听]/',
        '/storage/emulated/0/lz/直播[TV]/',
         '/storage/emulated/0/lz/novel/',
     ...
     * 获取随机封面图片
     */
    private function getRandomImage($index = 0)
    {
        $currentTime = time();
        $imgIndex = $index % count($this->defaultImages);
        return str_replace('{time}', $currentTime, $this->defaultImages[$imgIndex]);
    }

    public function getIcon($ext, $path)
    {
        // 对于目录、媒体文件、列表文件等返回随机封面图片
        if ($ext === 'dir' || is_dir($path)) {
            return $this->getRandomImage(crc32($path));
        }
        elseif (in_array($ext, $this->MEDIA_EXTS)) {
            return $this->getRandomImage(crc32($path));
        }
        elseif (in_array($ext, ['m3u', 'txt', 'json'])) {
            return $this->getRandomImage(crc32($path));
        }
        elseif ($ext === 'database' || $ext === 'db' || $ext === 'sqlite' || $ext === 'sqlite3') {
            return $this->getRandomImage(crc32($path));
        }
